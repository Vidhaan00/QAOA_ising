...
def gamma_layer(gamma_value, J, h):
    ...
    yield cirq.ZZ(qubits[-1], qubits[0]) ** (2 * gamma_value * J / np.pi)  # PBC
    for j in range(n_qubits):
        yield cirq.X(qubits[j]) ** (2 * gamma_value * h / np.pi)

def beta_layer(beta_value):
    for i in range(n_qubits - 1):
        yield cirq.XX(qubits[i], qubits[i+1]) ** (2 * beta_value / np.pi)
        yield cirq.YY(qubits[i], qubits[i+1]) ** (2 * beta_value / np.pi)
